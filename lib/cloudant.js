...
function findRecursive(mo, query, docs, include, cb) {
  mo.db.find(query, function(err, rst) {
    //console.log('findRecursive results, query: ' + JSON.stringify(query));
    debug('Cloudant.prototype.all (findRecursive) results: %j %j', err, rst);
    if (err) return cb(err);
    if (!rst.docs) {
      var error = {};
      error.message = "Malformed JSON response";
      error.statusCode = 500;
      console.log('Docs is undefined, query: ' + JSON.stringify(query));
      return cb(error);
    }
